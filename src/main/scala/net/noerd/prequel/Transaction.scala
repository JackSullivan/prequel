 ...
 * matically committed. If some exception is thrown during execution the 
 ...
 *
 * @throws IllegalFormatException: Will be thrown by all methods if the format 
 *         string is invalid or if there are not enough parameters.
 */
class Transaction( val connection: Connection, val formatter: SQLFormatter ) {
     ...
     * given block. All objects are kept in memory so this method is not suited
     * to very big result sets. Use selectAndProcess if you need to process 
     * bigger datasets.
     * 
